...
import awagadro.studentApp.Dao.StudentDAO;
...
public class Starter {
	public static void main(String[] args) {
		...
		// В списке вы увидите, что предметы пока не привязаны к профессиям - количество
		// = 0
		for (Subject a : sbList) {
			System.out.println(a.getSubjectId() + ":" + a.getSubjectName() + ". Number of profession:"
					+ a.getProfessionList().size());
		}
		...
		// В списке вы увидите, что предметы теперь привязаны к профессиям - количество
		// > 0
		for (Subject a : sbList) {
			System.out.println(a.getSubjectId() + ":" + a.getSubjectName() + ". Number of profession:"
					+ a.getProfessionList().size());
		...
		for (Applicant a : apList) {
			System.out
					.println(a.getFirstName() + ":" + a.getLastName() + " - " + a.getProfession().getProfessionName());
			// Если убрать комментарий, то получим сообщене об ошибке - коллекция не
			// инициализирована
